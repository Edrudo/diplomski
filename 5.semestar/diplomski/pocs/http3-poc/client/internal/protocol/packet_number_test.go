...
var _ = Describe(
	"packet number calculation", func() {
		It(
			"InvalidPacketNumber is smaller than all valid packet numbers", func() {
				Expect(InvalidPacketNumber).To(BeNumerically("<", 0))
			},
		)

		It(
			"works with the example from the draft", func() {
				Expect(DecodePacketNumber(PacketNumberLen2, 0xa82f30ea, 0x9b32)).To(Equal(PacketNumber(0xa82f9b32)))
			},
		)

		It(
			"works with the examples from the draft", func() {
		...
		check := func(length PacketNumberLen, expected, last uint64) {
			...
			ExpectWithOffset(
				1,
				DecodePacketNumber(length, PacketNumber(last), PacketNumber(wirePacketNumber)),
			).To(Equal(PacketNumber(expected)))
		}
		...
		for _, l := range []PacketNumberLen{PacketNumberLen1, PacketNumberLen2, PacketNumberLen3, PacketNumberLen4} {
			...
			Context(
				fmt.Sprintf("with %d bytes", length), func() {
					epoch := getEpoch(length)
					epochMask := epoch - 1

					It(
						"works near epoch start", func() {
					...
					It(
						"works near previous epoch", func() {
							prevEpoch := 1 * epoch
							curEpoch := 2 * epoch
							// Cases where the last number was close to the start of the range
							for i := uint64(0); i < 10; i++ {
					...
					Context(
						"shortening a packet number for the header", func() {
							Context(
								"shortening", func() {
									It(
										"sends out low packet numbers as 2 byte", func() {
									...
									It(
										"sends out high packet numbers as 2 byte, if all ACKs are received", func() {
									...
									It(
										"sends out higher packet numbers as 3 bytes, if a lot of ACKs are missing",
										func() {
									...
									It(
										"sends out higher packet numbers as 4 bytes, if a lot of ACKs are missing",
										func() {
							...
							Context(
								"self-consistency", func() {
									It(
										"works for small packet numbers", func() {
											for i := uint64(1); i < 10000; i++ {
												...
												decodedPacketNumber := DecodePacketNumber(
													length,
													leastUnacked,
													PacketNumber(wirePacketNumber),
												)
									...
									It(
										"works for small packet numbers and increasing ACKed packets", func() {
											for i := uint64(1); i < 10000; i++ {
												packetNumber := PacketNumber(i)
									...
									It(
										"also works for larger packet numbers", func() {
									...
									It(
										"works for packet numbers larger than 2^48", func() {
