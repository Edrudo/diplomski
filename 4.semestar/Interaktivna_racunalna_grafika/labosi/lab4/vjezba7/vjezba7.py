...
def on_draw():
    ...
    compute_constant_lighting(
        k_a=1, i_a=10, k_d=0.78125, i_p=256, k_c=0.1, k_s=0.17578125, n=5)
    ...
    for polygon in polygons:
        if polygon.is_visible():
            glColor3f(r * polygon.get_intensity(), g *
                      polygon.get_intensity(), b * polygon.get_intensity())
            glVertex3f(vertices[polygon.v_1-1][0],
                       vertices[polygon.v_1-1][1], vertices[polygon.v_1-1][2])
            glVertex3f(vertices[polygon.v_2-1][0],
                       vertices[polygon.v_2-1][1], vertices[polygon.v_2-1][2])
            glVertex3f(vertices[polygon.v_3-1][0],
                       vertices[polygon.v_3-1][1], vertices[polygon.v_3-1][2])
    glEnd()
...
def compute_constant_lighting(k_a, i_a, k_d, i_p, k_c, k_s, n) -> None:
    ...
    for polygon in polygons:
        if polygon.is_visible():
            ...
            # diffuse component
            dot_product = np.dot(polygon.n_hat, np.array(
    ...
    with open(filename) as f:
        for line in f.readlines():
            if line.startswith('v'):
                ...
                vertices.append(
                    np.array([float(coords[1]), float(coords[2]), float(coords[3]), 1]))
            if line.startswith('f'):
                polygon_indexes = line.strip().split(' ')
                p = Polygon(int(polygon_indexes[1]), int(
                    polygon_indexes[2]), int(polygon_indexes[3]), False)
                polygons.append(p)
            if line.startswith('#'):
                continue
            coords = line.strip().split(' ')
            coords = np.array(
                [float(coords[1]), float(coords[2]), float(coords[3])])
